...
- Calibrate against real Thai beauty market: TikTok dominance, storytelling content, KOL fatigue, trade-down in mid-2026 economy.`;
  ...
  async function callClaude({ model, system, userMessage, useSearch, useThinking }) {
    const tools = [];
    if (useSearch) {
  ...
  // Extract the final text response from Claude's content blocks
  // Skips thinking blocks and tool_use blocks; concatenates text blocks.
  ...
  // Robust JSON extraction (Claude sometimes wraps despite instructions)
...
- Bodies must explain BUYER BEHAVIOR impact, not just news.`;
  ...
  async function discoverTrends({ focusCategories = [], model = 'claude-sonnet-4-6' } = {}) {
    ...
    const userMessage = `Search the web and find current Thai beauty market trends that affect purchase decisions. Today is ${today}. Return JSON only.`;
